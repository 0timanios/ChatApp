...
const signup =async (req,res)=>{
    ...
    if(!fullName || !email || !password) return res.status(400).json({message: 'No field should be empty'})
...
const updateProfile = async(req,res) =>{
    ...
    try {
        ...
        console.log('Error in update profile conrtroller ', error.message)
        res.status(500).json({message:'Internal server error'})
    }
}
...
const checkAuth = (req, res) => {
    try {
        ...
        console.log('Error in check Auth conrtroller ', error.message)
        res.status(500).json({message:'Internal server error'})
    }
}
